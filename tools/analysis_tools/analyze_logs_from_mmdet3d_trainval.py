        ...
        for j, metric in enumerate(metrics):
            ...
            if args.mode == 'eval':
                ...
                for epoch in epochs[args.interval - 1::args.interval]:
                    ...
                    xs.append((np.array(iters) + (epoch - 1) * num_iters_per_epoch) / num_iters_per_epoch)
                    ys.append(np.array(log_dict[epoch][metric][:len(iters)]))
                xs = np.concatenate(xs)
                ys = np.concatenate(ys)
                #plt.xlabel('iter/(num iters per epoch)')
                plt.plot(
                    xs, ys, label="-".join([args.mode, metric]), linewidth=1.5, linestyle="-.")
            else:
                xs = []
                ys = []
                num_iters_per_epoch = \
                ...
                #plt.xlabel('iter/(num iters per epoch)')
                plt.plot(
                    xs, ys, label="-".join([args.mode, metric]), linewidth=1.5)
            #plt.legend()
    ...
    for json_log, log_dict in zip(json_logs, log_dicts):
        with open(json_log, 'r') as log_file:
            for line in log_file:
                ...
                if 'mode' not in log:
                    continue
                if log["mode"] != args.mode:
                    continue
                if args.mode != 'eval':
                    if 'memory' not in log:
                        continue
                else:
                    if 'memory' in log:
                        continue
