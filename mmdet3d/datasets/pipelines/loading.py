...
@PIPELINES.register_module()
class LoadPointsFromMultiFutureSweepsWaymo(LoadPointsFromMultiSweeps):
    """Load points from multiple sweeps.
    ...
    def __call__(self, results):
        ...
        # overlap past sweeps
        if self.pad_empty_sweeps and len(results['sweeps']) == 0:
            for i in range(self.sweeps_num[0]):
                if self.remove_close:
                    sweep_points_list.append(self._remove_close(points, self.close_radius))
                else:
                    sweep_points_list.append(points)
            ...
            if self.data_aug == True:
                if hasattr(self, 'sweep_choices'):
                    ...
                    choices = np.arange((len(results['sweeps'])-1)/2)
                else:
                    choices = np.arange(self.sweeps_num[0])
                choices = choices * 2 + 1
            else:
                if hasattr(self, 'sweep_choices'):
                    choices = self.sweep_choices
                elif len(results['sweeps']) <= self.sweeps_num[0]:
                    choices = np.arange(len(results['sweeps']))
                else:
                    choices = np.arange(self.sweeps_num[0])

            choices = choices.astype(np.int)
        ...
        if self.pad_empty_sweeps and len(results['sweeps_future']) == 0:
            ...
            if self.data_aug == True:
                if hasattr(self, 'sweep_choices'):
                    ...
                    choices = np.arange((len(results['sweeps_future'])-1)/2)
                else:
                ...
                if hasattr(self, 'sweep_choices'):
                    choices = self.sweep_choices
                elif len(results['sweeps_future']) <= self.sweeps_num[1]:
                    choices = np.arange(len(results['sweeps_future']))
                else:
                    choices = np.arange(self.sweeps_num[1])
            ...
            for idx in choices:
                ...
                if self.t_dim == points_sweep.shape[-1]:
                    ...
                    points_sweep[:, self.t_dim] =  float(idx+1)
...
@PIPELINES.register_module()
class LoadPointsFromMultiFutureSweepsWaymoAug(LoadPointsFromMultiSweeps):
    """Load points from multiple sweeps.
    ...
    def __call__(self, results):
        ...
        if p < 0.2:
            ...
            choices = random.sample(wait_list, len_choices)
        else:
            choices = wait_list
