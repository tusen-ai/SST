...
class Voxel2PointScatterNeck(nn.Module):
    """
    more memory-efficient voxel2point with torch_scatter
    """
    ...
    def forward(self, points, pts_coors, voxel_feats, voxel2point_inds, voxel_padding=-1):
        ...
        Args:
            ...
            point_coors (torch.Tensor): of shape (N, 4).
            voxel_feature (torch.Tensor): of shape (M, C_feature), should be padded and reordered.
            voxel2point_inds: (N,)
        ...
        if self.with_xyz:
            ...
            if self.training and not self.normalize_local_xyz:
                assert (local_xyz.abs() < voxel_size / 2 + 1e-3).all(), 'In test, this is not True because of lack of point range clip'
...
@NECKS.register_module()
class TemporalVoxel2PointNeck(nn.Module):
    """
    more memory-efficient voxel2point with torch_scatter
    """
    ...
    def __init__(
        ...
        self.normalize_local_xyz = normalize_local_xyz
        self.t_dim = t_dim
        assert len(voxel_size) == 4 and len(point_cloud_range) == 8
    ...
    def forward(self, points, pts_coors, voxel_feats, voxel2point_inds, voxel_padding=-1):
        ...
        if self.with_xyz:
            ...
            voxel_size = torch.tensor(self.voxel_size, dtype=dtype, device=device).reshape(1, 4)
            pc_min_range = torch.tensor(self.point_cloud_range[:4], dtype=dtype, device=device).reshape(1, 4)
            voxel_center_each_pts = (pts_coors[:, [3,2,1,4]].to(dtype).to(device) + 0.5) * voxel_size + pc_min_range# x y z order
            local_xyz = points[:, [0, 1, 2, self.t_dim]] - voxel_center_each_pts
            if self.normalize_local_xyz:
                local_xyz = local_xyz / (voxel_size / 2)
